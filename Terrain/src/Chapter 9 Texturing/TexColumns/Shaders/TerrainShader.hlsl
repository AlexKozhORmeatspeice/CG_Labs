...
Texture2D gHeightModificationMap : register(t3); // ���������� t3
...
PixelOut PS(VertexOut pin) : SV_Target
{
    ...
    float colorMod = gHeightModificationMap.SampleLevel(gSamLinearClamp, pin.TexC, 0).r;
    float gotColor = step(0.1f, colorMod);
    
    float4 secColor = float4(1.0f, 0.0f, 0.0f, 1.0f) * gotColor;
    
    pout.Albedo = diffuseAlbedo + gotColor;
    pout.Normal = float4(bumpedNormalW, gRoughness);
    pout.Position = float4(pin.PosW, 1.0f);
